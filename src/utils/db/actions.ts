...
export async function updateUser(userId: number, name: string, email: string) {
  try {
    const [updatedUser] = await db.update(Users).set({ name, email }).where(eq(Users.id, userId)).returning().execute();
    return updatedUser;
  } catch (error) {
...
export async function getRecentReports(limit: number = 10, setLoadReports : (loadReports : boolean) => void) {
  setLoadReports(true);
  try {
